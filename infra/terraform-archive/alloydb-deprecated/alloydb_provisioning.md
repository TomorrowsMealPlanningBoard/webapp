# AlloyDB(pgvector) プロビジョニングガイド (Issue #28) ※歴史記録

> ## ⚠️ 【撤去済み】このドキュメントは歴史的経緯として残しています
> **アーキテクチャ再検討（#61 クローズ時、Epic #75）により、TomorrowsMeal は AlloyDB(pgvector) を採用しません。**
> データ基盤は **Agent Platform Memory Bank**（層3の好み学習）＋ 層1/層2/層3'の構造化DB（Firestore 等）という
> フルマネージド構成に移行しました。詳細は [SPEC.md](../../../SPEC.md) §3・§5・§6 を参照。
> 新データ基盤のセットアップ・IAM要件は [`docs/data_platform_setup.md`](../../../docs/data_platform_setup.md) を参照。
>
> **Issue #79 で `infra/terraform/` から本ディレクトリへ移動し、IaC本体は `.tf.disabled` にリネームしました。**
> 本ドキュメントの手順は当時の記録であり、今後 `terraform apply` すべきものではありません。

## 概要（歴史記録）

TomorrowsMeal の層3（レシピ事例の検索・pgvectorベクトル検索、SPEC.md §3）は
当初 AlloyDB for PostgreSQL 上で稼働する想定でした。本ドキュメントでは、
当時 `infra/terraform/`（現 `infra/terraform-archive/alloydb-deprecated/`）に用意したIaCコードを使って
AlloyDB クラスタをプロビジョニングする手順、pgvector拡張の有効化確認、
Auth Proxy(IAM認証)経由の接続確認手順を記録として残す。
...
この方針に合わせて、`infra/terraform-archive/alloydb-deprecated/alloydb.tf.disabled` は以下のように「壊しやすく」設定されています:
...
```bash
cd infra/terraform-archive/alloydb-deprecated
...
```bash
export ALLOYDB_INSTANCE_URI="$(terraform -chdir=infra/terraform-archive/alloydb-deprecated output -raw alloydb_primary_instance_name)"
...
`infra/terraform-archive/alloydb-deprecated/iam.tf.disabled` により、`.github/workflows/deploy.yml` がデプロイする
